...
class Epuck:
    ...
    def __init__(self, position=(0,0), r=0.48, bHorizontal = False, bMatplotlib=False, frontIR = 6):
        ...
        self.body.userData["frontIR"]=frontIR
        ...
        self.body.userData["frontIRAngles"] = [k*da-((nir-m)/2)*da-m*da/2 for k in range(nir)]
        self.body.userData["frontIRValues"] = [1 for i in range(frontIR)]
 
    def getIRs(self):
        return self.body.userData["frontIRValues"]
    ...
    def update(self):
        ...
        nir = self.frontIR 
        irangles = self.body.userData["frontIRAngles"]
        frontIRValues = self.body.userData["frontIRValues"]

        for k in range(nir):
            v = vrotate((1,0),body.angle + irangles[k]) 
            c = pos+[0.9*self.r*v[0],0.9*self.r*v[1]]
            cdist = pos+[self.maxdist*self.r*v[0],self.maxdist*self.r*v[1]]            
            self.callback.fixture = None
            world.RayCast(self.callback, c, cdist)
            if(self.callback.fixture != None):
                frontIRValues[k] = dist(c,self.callback.point) / self.maxdist
            else:
                frontIRValues[k] = 1
...
class CartPole:
    ...
    def __init__(self, position=(0,0), name="simple", bMatplotlib = False, d = 1, bHand = 0, collisionGroup=None):
        ...
        self.circle = createCircle(position, r=d*0.6, bDynamic=True, bMatplotlib = True, name="wheel")
        self.box = createBox( (position[0],position[1]+d*1.9), d*0.2, d*2, bDynamic=True)
